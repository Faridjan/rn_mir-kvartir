	...
	return variations.find((variation) => {
		return variation.get('attributes').every((attribute) => {
			const option = meta_data.find(
				(m) => m.get('id') === attribute.get('id') && m.get('name') === attribute.get('name'),
			)
			// Any options
			if (!option) {
				return true
		...
		return variation.update('attributes', (attrs) => {
			return attrs.map((attr) => {
				...
				const options = attributes
					...
					.get('options')
				const option = options.find((option) => option.get('name') === attr.get('option'))
...
export function checkOption(variations, meta_data, option) {
	...
	return !!_variations.find((variation) => {
		...
		const attribute = attributes.find((attribute) => {
			return (
				option.get('id') === attribute.get('id') && option.get('name') === attribute.get('name')
			)
		})
