...
export default function authReducer(state = initState, action = {}) {
	switch (action.type) {
		...
		case Actions.SIGN_UP_WITH_EMAIL_ERROR:
			...
			return state.set('pending', false).set('signUpError', fromJS(errorSignUp))
		...
		case Actions.CHANGE_PASSWORD:
			return state.set('pendingChangePassword', true).set('changePasswordError', fromJS(initError))
		...
		case Actions.FORGOT_PASSWORD:
			return state.set('pendingForgotPassword', true).set('forgotPasswordError', fromJS(initError))
			...
			if (action.payload && action.payload.auth) {
				...
				return initState.merge(
					fromJS({
						...
						shippingAddress: auth.get('shippingAddress') || fromJS(shippingAddressInit),
					}),
				)
			} else {
				return state
